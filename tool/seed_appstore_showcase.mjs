/**
 * Seeds polished App Store showcase data for a Nestly account via Firestore REST.
 *
 * Usage:
 *   node tool/seed_appstore_showcase.mjs [email]
 *
 * Auth: Firebase CLI login tokens (refresh → access), or FIREBASE_TOKEN.
 */
...
const KNOWN_UIDS = {
  'devtime3@gmail.com': '1r0yz8uL9QYCbNRhYjDABI5elDE3',
};

const FIREBASE_CLI_OAUTH = {
  client_id:
    '563584335869-fgrhgmd47bqnekij5i8b5pr03ho849e6.apps.googleusercontent.com',
  client_secret: 'jEQSV5qaKtrPWiZHkHVpiV4W',
};
...
function tsValue(date) {
  ...
  return {
    timestampValue: new Date(seconds * 1000 + nanos / 1e6).toISOString(),
  };
}
...
function loadRefreshToken() {
  ...
  const refresh =
    tokens.refresh_token || tokens.refreshToken || cfg.token?.refresh_token;
  if (!refresh) {
    throw new Error('Firebase CLI token missing refresh_token. Run: firebase login');
  }
  return refresh;
}
...
async function getDoc(accessToken, ...parts) {
  try {
    return await api(accessToken, 'GET', docPath(...parts));
  } catch (err) {
...
async function upsertDoc(accessToken, fields, ...parts) {
  return api(accessToken, 'PATCH', docPath(...parts), { fields });
}

async function listDocs(accessToken, ...parts) {
  const json = await api(accessToken, 'GET', docPath(...parts));
  return json?.documents || [];
}

async function deleteDoc(accessToken, name) {
  return api(accessToken, 'DELETE', name);
}

async function clearCollection(accessToken, ...parts) {
  const docs = await listDocs(accessToken, ...parts);
  for (const doc of docs) {
    await deleteDoc(accessToken, doc.name);
  }
}

async function writeMany(accessToken, collectionParts, docs) {
  for (const [id, fields] of Object.entries(docs)) {
    await upsertDoc(accessToken, fields, ...collectionParts, id);
  }
}
...
async function main() {
  ...
  const uid = KNOWN_UIDS[EMAIL];
  if (!uid) {
    throw new Error(
      `Unknown email ${EMAIL}. Add UID to KNOWN_UIDS or create the account first.`,
    );
  }
  ...
  if (!nestId) {
    ...
    console.log(`Created nest ${nestId} (${inviteCode})`);
  } else {
    const nestDoc = await getDoc(accessToken, 'nests', nestId);
  ...
  for (const name of collections) {
    await clearCollection(accessToken, 'nests', nestId, name);
  }

  const dad = uid;

  await writeMany(accessToken, ['nests', nestId, 'members'], {
    [dad]: {
  ...
  await writeMany(accessToken, ['nests', nestId, 'tasks'], {
    'task-1': {
  ...
  await writeMany(accessToken, ['nests', nestId, 'shoppingLists'], {
    'list-groceries': {
  ...
  await writeMany(accessToken, ['nests', nestId, 'shoppingItems'], {
    'item-1': {
  ...
  await writeMany(accessToken, ['nests', nestId, 'events'], {
    'event-1': {
      title: str('School drop-off'),
      memberId: str(dad),
      category: str('School'),
  ...
  await writeMany(accessToken, ['nests', nestId, 'expenses'], {
    'exp-1': {
  ...
  await writeMany(accessToken, ['nests', nestId, 'bills'], {
    'bill-1': {
  ...
  await writeMany(accessToken, ['nests', nestId, 'emergency'], {
    'em-1': {
  ...
  await writeMany(accessToken, ['nests', nestId, 'vault'], {
    'vault-1': {
      title: str('Passports'),
      category: str('IDs'),
  ...
  await writeMany(accessToken, ['nests', nestId, 'timeline'], {
    'tl-1': {
  ...
  await writeMany(accessToken, ['nests', nestId, 'meals'], {
    'meal-1': {
      weekday: int(1),
      mealType: str('Dinner'),
  ...
  await writeMany(accessToken, ['nests', nestId, 'care'], {
    'care-1': {
      title: str('Walk Milo'),
      category: str('Pet'),
      cadenceDays: int(1),
  ...
  await writeMany(accessToken, ['nests', nestId, 'careProfiles'], {
    ayaan: {
      memberId: str('ayaan'),
      medications: str(''),
  ...
  await writeMany(accessToken, ['nests', nestId, 'school'], {
    'school-1': {
